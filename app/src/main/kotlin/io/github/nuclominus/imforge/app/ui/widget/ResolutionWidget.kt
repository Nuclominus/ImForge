 ...
 * The resolutions are generated by scaling down the provided width and height by a percentage,
 * starting from 100% down to 1%, with a step of the provided percent value.
 * This results in a list of resolutions that are smaller or equal to the original size.
 *
...
private fun generateResolutions(baseResolution: Resolution, percent: Int = 5) =
    mutableListOf<Pair<Int, Int>>().apply {
        for (i in MAX_QUALITY_IN_PERCENT downTo MIN_QUALITY_IN_PERCENT step percent) {
            add(
                baseResolution.first * i / MAX_QUALITY_IN_PERCENT to baseResolution.second * i / MAX_QUALITY_IN_PERCENT
            )
        }
    }.toList()

private const val MAX_QUALITY_IN_PERCENT = 100
private const val MIN_QUALITY_IN_PERCENT = 1
